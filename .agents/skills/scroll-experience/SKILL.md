...
### Framer Motion Scroll (React — preferred for this project)
```jsx
...
function StackedCards({ cards }) {
  ...
  return (
    <div ref={containerRef} style={{ height: `${cards.length * 100}vh` }}>
      {cards.map((card, idx) => {
        // Each card enters from bottom as user scrolls through its "slot"
        ...
        return (
          <motion.div
            key={card.id}
            style={{ y, position: 'sticky', top: 0, zIndex: idx + 1 }}
          >
            {card.content}
          </motion.div>
        );
...
- Step-by-step processes (like apartment viewing)
- Image galleries / stacked card reveals

## Anti-Patterns

### ❌ Scroll Hijacking
**Why bad**: Users hate losing scroll control.
Accessibility nightmare. Breaks back button expectations. Frustrating on mobile.

**Instead**: Enhance scroll, don't replace it.
Keep natural scroll speed. Use scrub animations. Allow users to scroll normally.

### ❌ Animation Overload
**Why bad**: Distracting, not delightful.
Performance tanks. Content becomes secondary. User fatigue.
...
Touch scroll is different. Performance issues on phones. Unusable experience.

**Instead**: Mobile-first scroll design.
Simpler effects on mobile. Test on real devices. Graceful degradation.

## ⚠️ Sharp Edges
| Issue | Severity | Solution |
|-------|----------|----------|
| Animations stutter during scroll | high | Use `will-change: transform`, GPU-accelerated props only |
| Parallax breaks on mobile devices | high | Use `useScroll` with `target` ref instead of global scroll |
| Scroll experience is inaccessible | medium | Respect `prefers-reduced-motion` |
| Critical content hidden below animations | medium | Content-first, animation as enhancement |
